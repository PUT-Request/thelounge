...
- Deliberately not upgraded, with reasons: chalk 5 is ESM-only and the server is CommonJS; TypeScript 7 was attempted and reverted: it removes `node10` module resolution (forcing `.js` extensions repo-wide plus dynamic imports for the now ESM-only commander) and its parser range (`@typescript-eslint` 8 supports TS `<6.1.0`) drags in the ESLint 9 flat-config migration — a full toolchain project, not a version bump; filenamify 7 renames on-disk log folders; read 5 and package-json 10 break their callback/metadata APIs; ldapjs stays because the LDAP test harness needs its in-process server (ldapts has none); got stays because link prefetch depends on its streaming abort and local-address bind semantics; web-push-neo is kept (it is newer than web-push); irc-framework stays pinned (the ping-timer fix is already in the pinned commit; the residual reconnect-counter quirk is narrow and not worth a local patch without Yarn Berry's patch protocol); vue-router 5, cssnano/postcss majors, and Prettier 3 / ESLint 9 are deferred as high-churn, zero-user-value migrations (cssnano/postcss carry visual-regression risk with no visual test coverage).
